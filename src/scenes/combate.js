export default class Combate extends Phaser.Scene {
    ...
    create() {
        ...
        try {
            ...
            // Enemy Sprite - Posición ajustada (Frontal) - Arriba Derecha
            ...
            let enemyScale = 0.8;
            let enemyX = 600;
            let enemyY = 220; // Bajamos un poco para que pise el "suelo" del fondo
            ...
            this.enemySprite = this.add.sprite(enemyX, enemyY, enemyKey).setScale(enemyScale);
            if (config) {
            ...
            let playerScale = 0.8;
            let playerX = 200;
            let playerY = 400; // Más abajo e izquierda
            ...
            this.playerSprite = this.add.sprite(playerX, playerY, backKey).setScale(playerScale);
            if (pConfig) {
    ...
    crearInterfazCombate() {
        ...
        this.enemyHUD = this.add.container(50, 40);
        this.enemyHUD.setDepth(15);

        // Base
        this.enemyHUD.add(this.add.rectangle(0, 0, 400, 110, 0x000000, 0.6).setOrigin(0));
        // Nombre y Nivel
        this.enemyNameText = this.add.text(20, 15, `${this.enemyPokemon.nombre} Lv.${this.enemyPokemon.nivel}`, { fontSize: '28px', fill: '#fff', fontStyle: 'bold' });
        this.enemyHUD.add(this.enemyNameText);
        // Barra Vida Fondo
        this.enemyHUD.add(this.add.rectangle(20, 60, 360, 30, 0x555555).setOrigin(0));
        // Barra Vida Actual
        this.enemyHPBar = this.add.rectangle(20, 60, 360, 30, 0x00ff00).setOrigin(0);
        this.enemyHUD.add(this.enemyHPBar);

        // --- Interfaz Jugador (Encima del menú, lado derecho) ---
        // Ajustamos posición para 800x600
        this.playerHUD = this.add.container(350, 330);
        this.playerHUD.setDepth(15);

        // Base
        this.playerHUD.add(this.add.rectangle(0, 0, 400, 110, 0x000000, 0.6).setOrigin(0));
        // Nombre y Nivel
        this.playerNameText = this.add.text(20, 15, `${this.currentPokemon.nombre} Lv.${this.currentPokemon.nivel}`, { fontSize: '28px', fill: '#fff', fontStyle: 'bold' });
        this.playerHUD.add(this.playerNameText);
        // Barra Vida Fondo
        this.playerHUD.add(this.add.rectangle(20, 60, 360, 30, 0x555555).setOrigin(0));
        // Barra Vida Actual
        this.playerHPBar = this.add.rectangle(20, 60, 360, 30, 0x00ff00).setOrigin(0);
        this.playerHUD.add(this.playerHPBar);
        // Texto HP Numérico
        this.playerHPText = this.add.text(380, 95, `${this.currentPokemon.vida}/${this.currentPokemon.vidaMax}`, { fontSize: '24px', fill: '#fff', fontStyle: 'bold' }).setOrigin(1, 0);
    ...
    actualizarBarrasVida() {
        ...
        this.enemyHPBar.width = 360 * Math.max(0, enemyPerfil);
        ...
        this.playerHPBar.width = 360 * Math.max(0, playerPerfil);
    ...
    crearMenuPrincipal() {
        ...
        const xBase = 450;
        const yBase = 460;
        const spacingX = 170; // Más espacio horizontal
        const spacingY = 65;  // Más espacio vertical

        this.crearBoton(this.menuGroup, xBase, yBase, 'Luchar', 0xff0000, () => this.mostrarMovimientos());
        this.crearBoton(this.menuGroup, xBase + spacingX, yBase, 'Cambiar', 0x0000ff, () => this.cambiarPokemon());
        this.crearBoton(this.menuGroup, xBase, yBase + spacingY, 'Mochila', 0x00aa00, () => this.usarObjeto());
        this.crearBoton(this.menuGroup, xBase + spacingX, yBase + spacingY, 'Huir', 0xaaaa00, () => this.huir());
    }
    ...
    mostrarMovimientos() {
        ...
        this.currentPokemon.movimientos.forEach((mov, index) => {
            // Fondo botón ataque - Más grande
            const btnBg = this.add.rectangle(x, y, 160, 55, 0xffffff)
                .setOrigin(0)
                .setInteractive({ useHandCursor: true })
                .on('pointerdown', () => {
            ...
            const btnText = this.add.text(x + 80, y + 20, mov.nombre, { fontSize: '20px', fill: '#000', fontStyle: 'bold' }).setOrigin(0.5);
            const tipoText = this.add.text(x + 80, y + 42, mov.tipo.toUpperCase(), { fontSize: '14px', fill: '#555' }).setOrigin(0.5);
