...
export default class Jugador extends Phaser.Physics.Arcade.Sprite {
    constructor(scene, x, y, party = []) {
        ...
        // Inicializar equipo Pokemon
        this.pokemonParty = party;
        if (this.pokemonParty.length === 0) {
            console.log("Jugador: No hay equipo, añadiendo inicial por defecto (Blastoise)");
            const starter = new Pokevon({ ...POKEVONES_INICIALES.blastoise, nivel: 5 });
            starter.esDelJugador = true;
            this.pokemonParty.push(starter);
        }
    ...
    moveTo(targetX, targetY) {
        ...
        this.scene.tweens.add({
            ...
            onComplete: () => {
                ...
                } else if (cellType === 4 || cellType === 5) { // Hierba (4) o Zona Combate Fijo (5)
                    console.log("⚔️ ¡Combate iniciado!");
                    if (this.scene.startWildEncounter) {
                        this.scene.startWildEncounter();
                    } else {
                        console.error("No se encontró la función startWildEncounter en la escena");
                    }
                } else {
                    // Si no es hierba, reiniciamos el contador (aunque ya no lo usamos para esto, good practice)
                    this.distanceTraveledInGrass = 0;
                }
            }
        });
    }
